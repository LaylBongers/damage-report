...
impl Texture {
    pub fn new<P: Into<PathBuf>>(path: P, format: TextureFormat) -> Arc<Self> {
        ...
        //  allows calcium to provide non-blocking texture loading. Sometimes this means that during
        //  rendering a model has to be skipped (or rendered without texture, provide choice!)
        //  because there's no texture loaded yet. The game using calcium needs to be able to detect
